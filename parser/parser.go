...
func (p *parser) parse() (*ast.File, error) {
	...
	var directives []*ast.Token
	var lastDirective *ast.Token
...
loop:
	for {
		...
		case ast.DIRECTIVE:
			p.next()
			// it is only a file directive if it is the first thing
			if len(directives) == 0 && len(file.Imports) == 0 && len(file.Stms) == 0 {
				// even if it is at the top check if there is white space after the directives
				if lastDirective == nil || lastDirective.Pos.Line == t.Pos.Line-1 {
					file.Directives = append(file.Directives, t.Str)
					lastDirective = t
					continue
				}
			}
		...
		case ast.IMPORT:
			if len(directives) > 0 {
				return nil, NewError(directives[0].Pos, "invalid directive")
			}
		...
		case ast.FUNCTION:
			...
			if len(directives) > 0 {
				for _, d := range directives {
					fnDec.Directives = append(fnDec.Directives, d.Str)
				}
				directives = nil
			} else if len(file.Stms) == 0 {
				if lastDirective != nil && lastDirective.Pos.Line == t.Pos.Line-1 {
					// if the directives are associated to the first function, move them
					// from the file to the function
					fnDec.Directives = file.Directives
					file.Directives = nil
					lastDirective = nil
				}
			}
		...
		case ast.CLASS:
			...
			if len(directives) > 0 {
				for _, d := range directives {
					classStmt.Directives = append(classStmt.Directives, d.Str)
				}
				directives = nil
			} else if len(file.Stms) == 0 {
				if lastDirective != nil && lastDirective.Pos.Line == t.Pos.Line-1 {
					// if the directives are associated to the first class, move them
					// from the file to the class
					classStmt.Directives = file.Directives
					file.Directives = nil
					lastDirective = nil
				}
			}
		...
		case ast.EXPORT:
			...
			if exp != nil {
				...
				case *ast.FuncDeclStmt:
					if len(directives) > 0 {
						for _, d := range directives {
							t.Directives = append(t.Directives, d.Str)
						}
						directives = nil
					} else if len(file.Stms) == 0 {
						if lastDirective != nil && lastDirective.Pos.Line == t.Pos.Line-1 {
