...
func NewWriter(q Query, database, driver string) *writer {
	if driver == "" {
		driver = "mysql"
	}
...
func ToSql(q Query, database, driver string) (string, []interface{}, error) {
	return NewWriter(q, database, driver).Write()
}
