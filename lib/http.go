...
declare namespace http {
	...
	export function newResponseRecorder(): ResponseWriter
...
var HTTP = []dune.NativeFunction{
	...
	},
	{
		...
		Function: func(this dune.Value, args []dune.Value, vm *dune.VM) (dune.Value, error) {
			return dune.NewString("application/x-javascript"), nil
		},
	},
	{
		Name: "->http.CONTENT_TYPE_JAVASCRIPT",
	...
	},
	{
		...
		Arguments: 0,
		Function: func(this dune.Value, args []dune.Value, vm *dune.VM) (dune.Value, error) {
			w := &responseWriter{writer: httptest.NewRecorder()}
...
func (s *server) SetProperty(name string, v dune.Value, vm *dune.VM) error {
	...
	case "handler":
		...
		case dune.Object:
			if t, ok := v.ToObject().(*dune.Closure); ok {
				s.closureHandler = t
			}
		default:
			return fmt.Errorf("invalid type, expected a function, got %v", v.TypeName())
		}
		return nil
...
func (s *server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	vm := s.vm.Clone(s.vm.Program, s.vm.Globals())
...
func (r *responseWriter) writeHTML(args []dune.Value, vm *dune.VM) (dune.Value, error) {
	if len(args) != 1 {
		return dune.NullValue, fmt.Errorf("expected 1 argument, got %d", len(args))
	}
	var a = args[0]
