...
var libArray = []dune.NativeFunction{
	...
	},
	{
		...
		Function: func(this dune.Value, args []dune.Value, vm *dune.VM) (dune.Value, error) {
			...
			case dune.Object:
				...
				if !ok {
					return dune.NullValue, fmt.Errorf("expected a function, got %s", b.TypeName())
				}
				closure = c
	...
	},
	{
		...
		Function: func(this dune.Value, args []dune.Value, vm *dune.VM) (dune.Value, error) {
			...
			sep := args[0]
			if sep.Type != dune.String {
				return dune.NullValue, fmt.Errorf("expected string, got %s", sep.TypeName())
			}
