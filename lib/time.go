...
declare namespace time {
    ...
    export function isDayOfWeekActive(value: number, t: Time | number): boolean
...
var Time = []dune.NativeFunction{
	...
	},
	{
		...
		Arguments: 3,
		Function: func(this dune.Value, args []dune.Value, vm *dune.VM) (dune.Value, error) {
			if err := ValidateArgs(args, dune.Int, dune.Int, dune.Bool); err != nil {
			...
			return dune.NewInt64(int64(value)), nil
		},
	},
	{
		...
		Function: func(this dune.Value, args []dune.Value, vm *dune.VM) (dune.Value, error) {
			if args[0].Type != dune.Int {
				return dune.NullValue, fmt.Errorf("expected argument 1 to be int, got %s", args[0].Type)
			}

			value := uint64(args[0].ToInt())

			// the day of week can be pased as a number or as a date. Prefer a date because it
			...
			case dune.Int:
				dayOfWeek = uint(args[1].ToInt())
			case dune.Object:
				t, ok := args[1].ToObject().(TimeObj)
				if !ok {
					return dune.NullValue, fmt.Errorf("expected time.Time, got %s", args[0].TypeName())
				}
				// the day must be in local time
				loc := GetLocation(vm)
				dayOfWeek = uint(time.Time(t).In(loc).Weekday())
			default:
				return dune.NullValue, fmt.Errorf("expected argument 2 to be WeekDay or time.Time, got %s", args[1].Type)
			}

			active := ((value >> dayOfWeek) & 1) == 1
			return dune.NewBool(active), nil
		},
	},
	{
		Name: "->time.Monday",
		...
		Function: func(this dune.Value, args []dune.Value, vm *dune.VM) (dune.Value, error) {
			...
			case dune.Object:
				if _, ok := v.ToObjectOrNil().(*dune.Closure); !ok {
					return dune.NullValue, fmt.Errorf("expected a function, got: %s", v.TypeName())
				}
...
func (l location) GetProperty(name string, vm *dune.VM) (dune.Value, error) {
	switch name {
	case "name":
		return dune.NewString(l.l.String()), nil
	}
