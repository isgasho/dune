...
func (c *compiler) compileIndexExpr(t *ast.IndexExpr, dest *Address) (*Address, error) {
	...
	if t.Optional {
		c.addOptChaining()
		c.emit(op_gto, dest, left, index, t.Left.Position())
	} else {
...
func (c *compiler) compileBinaryExpr(t *ast.BinaryExpr, dest *Address) (*Address, error) {
	...
	case ast.NOR:
		return c.compileAndOrExpr(t, jumpIfNotNull, dest)
	}
...
func (c *compiler) compileEnumValueExpr(enumAddr *Address, key string, dest *Address, pos ast.Position) (*Address, error) {
	...
	c.emit(op_enu, dest, enumAddr, keyAddr, pos)
...
func (c *compiler) compileSelectorExpr(t *ast.SelectorExpr, dest *Address) (*Address, error) {
	...
	if ok {
		...
		if ok {
			...
			x = c.newTempRegister()
			c.emit(op_rnp, x, addr, Void, t.X.Position())
		}
	}
	...
	if t.Optional {
		c.addOptChaining()
		c.emit(op_gto, dest, x, key, t.X.Position())
	} else {
...
func (c *compiler) addOptChaining() {
	...
	pc := c.pc() + 1
	i := c.emit(op_str, NewAddress(AddrData, 0), NewAddress(AddrData, pc), Void, ast.Position{})
...
func (c *compiler) compileTailCallStmt(t *ast.CallExpr) error {
	...
	for i := 0; i < f.Arguments; i++ {
		if lenArgs > i {
			c.emit(op_mov, NewAddress(AddrLocal, i), argRegs[i], Void, t.Position())
		} else {
			c.emit(op_mov, NewAddress(AddrLocal, i), Void, Void, t.Position())
		}
	}

	// jump to the beginning of the function
	c.emit(op_jpb, NewAddress(AddrData, c.pc()), Void, Void, t.Position())
...
func (c *compiler) compileCallExpr(t *ast.CallExpr, dest *Address, retVal bool) (*Address, error) {
	...
	if !t.Spread && len(t.Args) == 1 {
		...
		if t.Optional {
			c.addOptChaining()
			c.emit(op_cso, i, dest, exp, t.Position())
		} else {
	...
	if t.Optional {
		c.addOptChaining()
		c.emit(op_cco, i, dest, args, t.Position())
	} else {
