...
func writeClass(w io.Writer, classes []*dune.Class, key byte) error {
	...
	for _, c := range classes {
		if err := writeDirectives(w, c.Directives, key); err != nil {
			return err
		}
...
func writeFunctions(w io.Writer, funcs []*dune.Function, key byte) error {
	...
	for _, f := range funcs {
		...
		if err := writeDirectives(w, f.Directives, key); err != nil {
			return err
		}
		if err := writeString(w, f.Name, key); err != nil {
			return err
...
func writeDirectives(w io.Writer, directives []string, key byte) error {
