...
func readDirectives(r io.Reader, key byte) ([]string, error) {
	s, err := readSection(r)
	if err != nil {
		return nil, err
	}
...
func readClasses(r io.Reader, key byte, p *dune.Program) error {
	...
	for i, l := 0, int(v); i < l; i++ {
		...
		if class.Directives, err = readDirectives(r, key); err != nil {
			return err
		}
...
func readFunctions(r io.Reader, key byte, p *dune.Program) error {
	...
	for i, l := 0, int(v); i < l; i++ {
		...
		if f.Directives, err = readDirectives(r, key); err != nil {
			return err
		}
		if f.Name, err = readString(r, key); err != nil {
			return err
